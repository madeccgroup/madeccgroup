 ...
 */
app.get("/api/cloudinary-signature", async (req: Request, res: Response) => {
  try {
    const ticket = createCloudinarySignedUploadTicket();
    ...
    return res.status(500).json({
      message: "Unable to obtain Cloudinary upload signature.",
    });
  }
});
